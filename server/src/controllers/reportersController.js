...
export const getReporters = async (req, res, next) => {
  try {
    ...
    const formatted = result.rows.map(row => ({
      ...
      // Note: updatedBy and updatedByName are NOT in Firebase structure
    }));
    res.json(formatted);
  } catch (error) {
    next(error);
...
export const deleteReporter = async (req, res, next) => {
  try {
    ...
    emitReporterChange(req, 'deleted', { id: result.rows[0].id });
